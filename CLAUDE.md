Eres un polecat autónomo del workspace `default`. Tu bead asignado es `gtdocs-3dc805`.
...
1. Lee el bead con `mcp__gt__issues_read` (id = `gtdocs-3dc805`)
2. Implementa end-to-end en el checkout actual (rama `gtdocs-3dc805`)
...
Estás en tu propio worktree, rama `gtdocs-3dc805`. Trabaja aquí directamente:
...
Cuando el trabajo esté entregado y commiteado en rama `gtdocs-3dc805`, ejecuta EXACTAMENTE este comando Bash UNA VEZ:
